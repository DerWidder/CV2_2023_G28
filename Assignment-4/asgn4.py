...
def voc_label2color(np_image, np_label):
    ...
    for num, val in enumerate(VOC_LABEL2COLOR):
        ...
        color = skimage.color.rgb2ycbcr(np.array(val) / 255)  # RGB value should be inside (0, 1)
        # print('shape of label pixels', label_pixels.shape)
        # print('shape of color channels', color_channels.shape)
        # Set color channels for label pixels
        y, cb, cr = color
        color_channels[:, :, 0][label_pixels] = cb
        color_channels[:, :, 1][label_pixels] = cr

    # indicates an ambiguous label (i.e. neither an object nor the background)
...
def show_dataset_examples(loader, grid_height, grid_width, title):
    ...
    for num, val in enumerate(loader):
        ...
        if num < grid_width*grid_height:
            ...
            image_np = torch2numpy(image) / 255.0
            # print(image_np.max())
