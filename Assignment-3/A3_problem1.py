...
def alpha_expansion(noisy, init, edges, candidate_pixel_values, s, lmbda):
    ...
    for i in candidate_pixel_values:  # iterate value from 0 to 254
        ...
        unary_r2 = mrf_denoising_nllh(denoised, i * np.ones_like(noisy), s).reshape(1,
                                                                                    H * W)  # create second row of unary
        ...
        for j in range(edges.shape[0]):  # update the array pairwise
            ...
            if denoised_pixel_value[index_x] != denoised_pixel_value[
                index_y]:  # Potts model, if the value of two nodes of an edge are not same
        ...
        if psnr_after > psnr_before:  # check if get a higher psnr_after, then break the loop
...
def compute_psnr(img1, img2):
    ...
    v_max = 254
    # v_max should be 254, as said in the discussion forum
