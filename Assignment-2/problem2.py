...
def log_gaussian(x,  mu, sigma):
    ...
    value = np.sum(result)   # 直接在这一问给value求和
...
def stereo(d0, im0, im1, mu, sigma, alpha, method=optim_method()):
    ...
    x0 = d0.flatten()
    # res = optimize.minimize(fun(args), x0, method=method, jac=grad(args), tol=1)
    res = optimize.minimize(fun(args), x0, method=method, jac=grad(args), tol=1e-3)
    print(res.success)
...
def upsample_x2(x, factor=2):
    ...
    upsample = interpolate.griddata((x1, y1), newarr.ravel(), (xx, yy), method='cubic', fill_value=0)  # the boundaries are set to be zero
...
def gaussian_pyramid(img, nlevels, fsize, sigma):
    ...
    for i in range(nlevels):
        if i == 0:
            ...
            temp = downsample_x2(signal.convolve2d(GP[i - 1], kernel, mode='same'))  # first filtering and then downsampling
...
def main():
    ...
    d0 = gt
    # d0 =  gtrandom_disparity(gt.shape)
