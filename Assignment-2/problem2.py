...
def shift_interpolated_disparity(im1, d):
    ...
    for i in range(H):
        ...
        # for j in range(np.size(points)):
        #     if points_shifted[j] < 0:
        #         points_shifted[j] = 0
        for index, value in enumerate(points_shifted):
            if value < 0:
                points_shifted[index] = 0

        # shifted_im1[i, :] = f_inter(points - d[i, :])
...
def stereo(d0, im0, im1, mu, sigma, alpha, method=optim_method()):
    ...
    x0 = d0.flatten()
    res = optimize.minimize(fun(args), x0, method=method, jac=grad(args))
    print(res.success)
...
def upsample_x2(x, factor=2):
    ...
    upsample = np.zeros((2*H, 2*W))
    upsample[::factor, ::factor] = x
    # 这里应该对其余点进行插值，我按上面网页的方法失败了，需要再考虑该怎么写
    # 理论上也可以复制，让新的四个点跟原来的一个点相同，但是效果应该没有差值好，应该是可以写一个卷积核的
